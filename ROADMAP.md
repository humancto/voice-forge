...
- [ ] **6.2 `voiceforge pack {list,install,remove,info}` subcommand. Pulls index from `humancto/voice-forge-packs` over HTTPS, sha256-verifies, extracts to `~/.voiceforge/packs/<name>/`. Index already live (see 6.3).**
...
- [ ] **6.4 Initial pack release v1: 5 voices × 13 events shipped to `voice-forge-packs`. Peter shipped; queued (phrase manifests authored, awaiting clean reference clips): stewie, neil_tyson, bob_ross, herzog, ramsay, kimmel, trump, obama. Each tagged with `tier` (`character` / `public-figure` / `experimental`) in manifest.toml.**
...
- [ ] **6.6 Pack-content style guide (`docs/PACK_CONTENT_GUIDE.md`): per-tier rules for phrase content. `character` packs lean into in-character idioms; `public-figure` packs use clearly-synthetic dev-feedback framing (no fake political statements, no fake endorsements, no anything that could read as "this person actually said this"). Required reading for any community pack PR.**
- [ ] **6.7 Tarball release automation: GitHub Action in `voice-forge-packs` that builds `<voice>.tar.gz` per pack on tag push, computes sha256, attaches to release. Consumed by `voiceforge pack install <name>` (6.2).**
